...
func TestSharedServiceConfigs_StampConfigHash(t *testing.T) {
	cases := map[string]runtime.ContainerConfig{
		"mail":          MailContainerConfig(MailServiceConfig{Image: "axllent/mailpit:latest", Domain: "0ploy.dev"}),
		"db":            DBUIContainerConfig(DBUIServiceConfig{Image: "adminer:latest", Domain: "0ploy.dev"}),
		"redisInsights": RedisInsightsContainerConfig(RedisInsightsServiceConfig{Image: "redis/redisinsight:latest", Domain: "0ploy.dev"}),
		"router":        RouterContainerConfig(RouterConfig{Image: config.RouterImage, Domain: "0ploy.dev"}),
		"logs":          LogsContainerConfig(LogsServiceConfig{Image: "amir20/dozzle:latest", Domain: "0ploy.dev"}),
	}
...
func TestMailContainerConfig_HashChangesOnTLSFlip(t *testing.T) {
	noTLS := MailContainerConfig(MailServiceConfig{Image: "axllent/mailpit:latest", Domain: "0ploy.dev", TLSEnabled: false})
	withTLS := MailContainerConfig(MailServiceConfig{Image: "axllent/mailpit:latest", Domain: "0ploy.dev", TLSEnabled: true})
...
func TestDBUIContainerConfig_HashChangesOnTLSFlip(t *testing.T) {
	noTLS := DBUIContainerConfig(DBUIServiceConfig{Image: "adminer:latest", Domain: "0ploy.dev", TLSEnabled: false})
	withTLS := DBUIContainerConfig(DBUIServiceConfig{Image: "adminer:latest", Domain: "0ploy.dev", TLSEnabled: true})
...
func TestMailContainerConfig_HashChangesOnImageBump(t *testing.T) {
	v1 := MailContainerConfig(MailServiceConfig{Image: "axllent/mailpit:v1.0", Domain: "0ploy.dev"})
	v2 := MailContainerConfig(MailServiceConfig{Image: "axllent/mailpit:v2.0", Domain: "0ploy.dev"})
...
func TestStartService_NoDriftNoRecreate(t *testing.T) {
	...
	m := &Manager{
		cfg:     &config.GlobalConfig{Shared: config.SharedConfig{Mail: config.MailConfig{Image: "axllent/mailpit:latest"}}, Domain: "0ploy.dev"},
		runtime: mock,
	}
	mock.NetworksExist[SharedNetworkName] = true

	// Seed a "running" mail container with the current expected config.
	seeded := MailContainerConfig(MailServiceConfig{Image: "axllent/mailpit:latest", Domain: "0ploy.dev"})
...
func TestStartService_HashMismatchRecreates(t *testing.T) {
	...
	stale := MailContainerConfig(MailServiceConfig{Image: "axllent/mailpit:latest", Domain: "0ploy.dev", TLSEnabled: false})
	...
	if created.Labels["traefik.http.routers.zdev-mail-https.tls"] != "true" {
